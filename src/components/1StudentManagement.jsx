    ...
    <div className="flex flex-col gap-6">
      ...
      {showConfig && (
        <div className="bg-white p-6 rounded-lg shadow border border-gray-200 mb-6 animate-fade-in-down">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-lg text-gray-800 flex items-center gap-2"><Settings size={20}/> 환경 설정 (학교, 학년, 레벨 관리)</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            {/* 레벨 관리 */}
            <div className="flex flex-col gap-2">
              <h4 className="text-sm font-bold text-gray-600">레벨(Level) 목록</h4>
              <div className="flex flex-wrap gap-2 mb-2">
                {safeLevels.map(c => (
                  <span key={c} className="bg-gray-100 px-3 py-1 rounded-full text-sm flex items-center gap-2 border">
                    {c} <button onClick={() => handleDeleteLevel(c)} className="text-gray-400 hover:text-red-500 bg-transparent"><X size={12}/></button>
                  </span>
                ))}
              </div>
              <div className="flex gap-2">
                <input type="text" placeholder="예: Adv-A" className="border p-2 rounded text-sm flex-1 bg-white" value={newLevelInput} onChange={(e) => setNewLevelInput(e.target.value)}/>
                <button onClick={handleAddLevel} className="bg-gray-600 text-white px-3 py-1 rounded text-sm hover:bg-gray-700">추가</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow border border-indigo-200 bg-indigo-50 relative">
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex flex-col gap-1"><label className="text-xs font-bold text-gray-600">Level</label><select className="border p-2 rounded w-32 bg-white text-gray-900" value={safeNewStudent.classInfo || ''} onChange={(e) => setNewStudent({...safeNewStudent, classInfo: e.target.value})}>{safeLevels.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
      ...
      <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
        ...
        <table className="w-full text-sm text-left"><thead className="bg-gray-100"><tr><th className="p-2 border w-16">No.</th><th className="p-2 border">이름 (클릭하여 관리)</th><th className="p-2 border">학교</th><th className="p-2 border">학년</th><th className="p-2 border">Level</th><th className="p-2 border w-20">관리</th></tr></thead>
